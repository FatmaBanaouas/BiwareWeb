...
export default function JobsSection() {
  ...
  const handleApply = (jobId) => navigate(`/postuler/${jobId}`);
  ...
  return (
    <section id="carrieres" style={{ padding: '7rem 3rem', background: 'var(--bw-darker)' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        <div>
          ...
          <h2 style={{
            ...
            {t('jobs.title').split(' ').slice(0, 5).join(' ')}<br />
            {t('jobs.title').split(' ').slice(5, 7).join(' ')} <span style={{ color: 'var(--bw-orange)' }}>{t('jobs.title').split(' ').slice(7).join(' ')}</span>
          </h2>
        ...
        {jobs.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '3rem', marginTop: '3.5rem' }}>
            <p style={{ color: 'rgba(255,255,255,0.4)' }}>{t('jobs.no_offers')}</p>
          </div>
        ) : (
          <div className="jobs-grid" style={{
            ...
            {jobs.map((job) => (
              ...
              >
                <div style={{
                  display: 'inline-block', padding: '0.25rem 0.85rem', borderRadius: 30,
                ...
                <div style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.38)', lineHeight: 1.7, marginBottom: '1.25rem' }}>
                  {job.shortDesc}
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', color: 'var(--bw-orange)', fontSize: '0.8rem', fontWeight: 600 }}>
                  <span>{t('jobs.apply')}</span>
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"
                    style={{ transition: 'transform 0.2s ease', transform: hoveredId === job.id ? 'translateX(4px)' : 'translateX(0)' }}>
        ...
        <div style={{
          ...
          <p style={{ fontSize: '0.85rem', color: 'rgba(255,255,255,0.4)', marginBottom: '1rem' }}>
            {t('jobs.spontaneous_title')}
          </p>
          <button
            onClick={() => navigate('/candidature-spontanee')}
            style={{
              background: 'transparent', border: '1px solid rgba(242,101,34,0.35)',
              color: 'var(--bw-orange)', padding: '0.7rem 1.8rem',
              borderRadius: 40, fontSize: '0.85rem', fontWeight: 500,
              cursor: 'pointer', transition: 'all 0.2s ease',
            }}
            onMouseEnter={(e) => { e.currentTarget.style.background = 'rgba(242,101,34,0.08)'; e.currentTarget.style.borderColor = 'var(--bw-orange)'; }}
            onMouseLeave={(e) => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.borderColor = 'rgba(242,101,34,0.35)'; }}
          >
            {t('jobs.spontaneous_button')}
          </button>
        </div>
      </div>
    </section>
